...
- 你的总结必须凝练简洁，related_information 字段的内容不得超过500字。只保留最核心的相关信息，去除冗余描述。
...
URL_FILTER_PROMPT = """你是一个 URL 相关性筛选专家。给定一个子问题和一组候选 URL（含标题和摘要），请选出与子问题最相关的 {top_k} 个 URL。
...
## 输出格式
请直接返回你选出的 URL 索引列表（从 0 开始），用 JSON 数组格式：
[0, 3, 5, 7]
...
- Your summary must be concise and condensed. The content of the related_information field must not exceed 500 words. Only retain the most essential relevant information and remove redundant descriptions.
