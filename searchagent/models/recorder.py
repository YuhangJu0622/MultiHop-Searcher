...
class Recorder:
    ...
    def update(self, node_name, node_content, content, memory, sender):
        ...
        if sender == 'planner':
            ...
            existing_memory = self.container['content'].nodes[node_name].get('memory', {})
            if not isinstance(existing_memory, dict):
                existing_memory = {}
            existing_memory.update(ref2url)
            self.container['content'].nodes[node_name]['memory'] = existing_memory
