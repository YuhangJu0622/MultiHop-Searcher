...
class Planner(BaseStreamingAgent):
    ...
    def plan(self, message: Union[List[Union[AgentMessage,str]],Union[AgentMessage,str]], recorder, session_id=0, ctx=None, **kwargs):
        """
        ...
        if isinstance(message, list):
            ...
            for response in super().forward(message[-1],session_id=session_id, ctx=ctx, **kwargs):
                yield response
            
        else:
            for response in super().forward(message,session_id=session_id, ctx=ctx, **kwargs):
                yield response
