import os, json, traceback, time, warnings, openai
...
from ..schema import (
    AgentMessage, GlobalTimeoutException, NonRetryableError,
)
...
class GPTAPI(BaseAPILLM):
    ...
    def chat(
        ...
        **gen_params,
        ...
        Returns:
            ChatCompletionMessage: generated message object
        """
        assert isinstance(inputs, list)
        if 'max_tokens' in gen_params:
            raise NotImplementedError('unsupported parameter: max_tokens')
        gen_params = {**self.gen_params, **gen_params}
        return self._chat(messages=inputs, tools=tools, tool_choice=tool_choice, **gen_params)
    ...
    def _chat(self, messages: List[dict], tools: List[Dict] = [], tool_choice: str = "auto", **gen_params) -> str:
        ...
        self.logger.debug(
            "[LLM _chat Request] caller=%s, model=%s, messages_count=%d, has_tools=%s",
            self.caller_name, self.model_type, len(messages), bool(tools)
        )
        if self.caller_name != 'reader':
            for i, msg in enumerate(messages):
                if isinstance(msg, dict):
                    role = msg.get('role', 'unknown')
                    content = msg.get('content', '')
                    if role == 'system':
                        self.logger.debug("[LLM _chat Request] message[%d] role=system, len=%d chars (template omitted)", i, len(content) if content else 0)
                    else:
                        self.logger.debug("[LLM _chat Request] message[%d] role=%s, content=%s", i, role, str(content) if content else '')

        ctx = gen_params.pop('ctx', None)
        ...
        while max_num_retries < self.retry:
            ...
            with Lock():
                if len(self.invalid_keys) == len(self.keys):
                    raise NonRetryableError(f'[LLM caller={self.caller_name}] All keys have insufficient quota.')
            ...
            try:
                ...
                _timeout_kwargs = {}
                if ctx:
                    _timeout_kwargs['timeout'] = min(ctx.remaining, 30)

                if tools:
                    response = openai.chat.completions.create(
                        messages=messages, tools=tools,
                        tool_choice=tool_choice, **data, **_timeout_kwargs
                    )
                else:
                    response = openai.chat.completions.create(
                        messages=messages, **data, **_timeout_kwargs
                    )

                _api_elapsed = time.time() - _api_start
                result = response.choices[0].message
                if result.tool_calls:
                    tool_info = [{"name": tc.function.name, "args": tc.function.arguments} for tc in result.tool_calls]
                    self.logger.debug("[LLM _chat Response] caller=%s, model=%s, elapsed=%.2fs, tool_calls=%s", self.caller_name, self.model_type, _api_elapsed, tool_info)
                elif self.caller_name == 'reader':
                    _resp_len = len(result.content) if result.content else 0
                    self.logger.debug("[LLM _chat Response] caller=%s, model=%s, elapsed=%.2fs, len=%d chars", self.caller_name, self.model_type, _api_elapsed, _resp_len)
                else:
                    self.logger.debug("[LLM _chat Response] caller=%s, model=%s, elapsed=%.2fs, content=%s", self.caller_name, self.model_type, _api_elapsed, str(result.content) if result.content else '')
                return result

            except (openai.AuthenticationError, openai.BadRequestError) as e:
                raise NonRetryableError(f'[LLM caller={self.caller_name}] {e}') from e

            except Exception as e:
                if ctx and ctx.expired:
                    raise GlobalTimeoutException(f"全局超时 (caller={self.caller_name}, model={self.model_type})") from e
                errmsg = f'{type(e).__name__}: {e}'
                self.logger.warning(
                    "[LLM _chat] caller=%s, Retryable (%d/%d): %s",
                    self.caller_name, max_num_retries + 1, self.retry, errmsg
                )
                time.sleep(0.5)

            max_num_retries += 1

        raise NonRetryableError(f'[LLM caller={self.caller_name}] API调用失败，已重试{max_num_retries}次: {errmsg}')
    ...
    def stream_chat(
        ...
        **gen_params,
        ...
        # DEBUG: 记录流式 LLM 最终响应（含耗时）
        _stream_elapsed = time.time() - _stream_start
        if isinstance(resp, str):
            self.logger.debug("[LLM stream_chat Response] caller=%s, model=%s, elapsed=%.2fs, content=%s", self.caller_name, self.model_type, _stream_elapsed, resp if resp else '')
        elif hasattr(resp, 'tool_calls') and resp.tool_calls:
            tool_info = [{"name": tc.function.name, "args": tc.function.arguments} for tc in resp.tool_calls]
            self.logger.debug("[LLM stream_chat Response] caller=%s, model=%s, elapsed=%.2fs, tool_calls=%s", self.caller_name, self.model_type, _stream_elapsed, tool_info)
        elif hasattr(resp, 'content'):
            self.logger.debug("[LLM stream_chat Response] caller=%s, model=%s, elapsed=%.2fs, content=%s", self.caller_name, self.model_type, _stream_elapsed, str(resp.content) if resp.content else '')
    ...
    def _stream_chat(self, messages: List[dict], tools: List[Dict] = [], tool_choice: str = "auto", **gen_params) -> str:
        ...
        self.logger.debug(
            "[LLM _stream_chat Request] caller=%s, model=%s, messages_count=%d, has_tools=%s",
            self.caller_name, self.model_type, len(messages), bool(tools)
        )
        if self.caller_name != 'reader':
            for i, msg in enumerate(messages):
                if isinstance(msg, dict):
                    ...
                    content = msg.get('content', '')
                    if role == 'system':
                        self.logger.debug("[LLM _stream_chat Request] message[%d] role=system, len=%d chars (template omitted)", i, len(content) if content else 0)
                    else:
                        self.logger.debug("[LLM _stream_chat Request] message[%d] role=%s, content=%s", i, role, str(content) if content else '')
        ...
        def streaming(raw_response, has_tool=False):
            try:
                if has_tool:
                    final_tool_calls = {}
        ...
        while max_num_retries < self.retry:
            ...
            try:
                ...
                if ctx:
                    ctx.register_connection(response)
                yield from streaming(response, has_tool=bool(tools))
                return
                ...
                errmsg = f'{type(e).__name__}: {e}'
                self.logger.warning(
                    "[LLM _stream_chat] caller=%s, Retryable (%d/%d): %s",
                    self.caller_name, max_num_retries + 1, self.retry, errmsg
                )
                time.sleep(0.5)
...
class StreamingAgentMixin:
        ...
        for message in messages:
            ...
            if isinstance(message, AgentMessage):
                ...
                if isinstance(message.content, ChatCompletionMessage):
                    _message.append(
                        self._sanitize_assistant_message(message.content))
    ...
    @staticmethod
    def _sanitize_assistant_message(msg: ChatCompletionMessage) -> dict:
        """将 ChatCompletionMessage 转为 dict，并确保 tool_call arguments 是合法 JSON。

        某些模型可能生成非标准 JSON 的 function.arguments，repair_json 可以
        在 Searcher 内部容错，但原始字符串若被原样送回 API 会触发
        InternalError.Algo.InvalidParameter。这里做最后一道防线。
        """
        ...
        if msg.tool_calls:
            ...
            msg_dict["tool_calls"] = sanitized_tool_calls
        return msg_dict
