...
logger.debug("[VisitPage] headers=%s", headers)
class VisitPage(BaseTool):
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        logger.warning("[VisitPage] Timeout: %s: TimeoutError", futures[future])
                        webpage_cache.store_failed(url=futures[future], e="TimeoutError")

                    except Exception as e:
                        logger.warning("[VisitPage] Error fetching %s: %s", futures[future], e)
    ...
    def need_proxy(self, url):
        try:
            ...
            logger.warning("[VisitPage] 直接请求失败, 降级到 serper.dev: %s, %s: %s", url, type(e).__name__, e)
            serper_api_key = self.api_key
            response = requests.get(f"https://scrape.serper.dev?url={url}&apiKey={serper_api_key}")
    ...
    def parse(self, response):
        ...
        if 'text/html' not in content_type:
            logger.warning("[VisitPage] Unsupported content type %s for %s", content_type, response.url)
            
        try:
            soup = BeautifulSoup(response.text, 'lxml')
            
        except Exception as e:
            logger.warning("[VisitPage] lxml 解析失败, 降级到 html.parser: %s, %s: %s", response.url, type(e).__name__, e)
    ...
    def parse_pdf(self, response):
        try:
            ...
            logger.warning("[VisitPage] PDF Process Error: %s : %s", e, response.url)
            return 
    ...
    def parse_docx(self, response):
        ...
        try:
            ...
            logger.warning("[VisitPage] DOCX Parse Error: %s - %s", response.url, e)
            return 
    ...
    def parse_excel(self, response):
        ...
        try:
            ...
            logger.warning("[VisitPage] Excel Parse Error: %s - %s", response.url, e)
            return 
